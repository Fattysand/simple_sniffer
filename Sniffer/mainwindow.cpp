    ...
    ui(new Ui::MainWindow)
{
    ...
    //隐藏列表头
    ui->showWidget->verticalHeader()->setVisible(false);    
    ...
    //获取当前设备网络接口
    if(sniff_initCap() < 0){
        QMessageBox::warning(this, tr("Sniffer"), tr("无法获取网络适配器接口"), QMessageBox::Ok);
    }
    //初始化接口列表
...
int MainWindow::sniff_startCap()
{
    ...
    if(filterContent == NULL)
    {
        ...
        char *filter = NULL;
        filter = ba.data();     //QString中不应含有中文，否则会乱码
        if(pcap_compile(adhandle, &fcode, filter, 1, netmask) < 0)
        {
            QMessageBox::warning(this, "Sniff", tr("无法编译包过滤器，请检查语法"), QMessageBox::Ok);
            pcap_freealldevs(alldevs);
