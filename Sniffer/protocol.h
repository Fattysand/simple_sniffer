...
#define IP_HL(ip)       ((ip)->ip_vhl & 0x0f)   //后4位，报文首部长度
...
#define TH_OFF(th) (((th)->th_offx2 & 0xf0) >> 4)   //前4位，包首部长度
    u_char th_flags;
...
typedef struct _icmphdr
{
    u_char type;        //8位类型字段
    u_char code;        //8位代码字段
    u_short chk_sum;    //16位校验和字段
    u_short identification; //16位标识符字段
    u_short seq;    //16位序列号字段
}icmphdr;
